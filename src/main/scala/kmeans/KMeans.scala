...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val map0 = means.map(x => (x, GenSeq[Point]())).toMap
    map0 ++ points.groupBy(p => findClosest(p, means))
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val conver = converged(eta)(means, newMeans)
    if (! conver) kMeans(points, newMeans, eta) else newMeans // your implementation need to be tail recursive
  }
}
